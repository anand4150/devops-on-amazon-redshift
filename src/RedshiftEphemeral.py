...
class RedshiftEphemeral:
    ...
    def create_cluster(self):
        ...
        waiter.wait(
            ClusterIdentifier=self.name,
            MaxRecords=99,
            WaiterConfig={"Delay": 10, "MaxAttempts": 99},
        )
        print("==================================================================")
    ...
    def delete_cluster(self):
        ...
        waiter.wait(
            ClusterIdentifier=self.name,
            MaxRecords=99,
            WaiterConfig={"Delay": 6, "MaxAttempts": 99},
        )
        print("===================================================")
    ...
    def pause_cluster(self):
        ...
        try:
            ...
            response = obj.pause_cluster(ClusterIdentifier=self.name)
        print("===================================================")
    ...
    def resume_cluster(self):
        ...
        try:
            response = obj.resume_cluster(ClusterIdentifier=self.name)
        except Exception as e:
            print(e)
        # poll cluster availability
        waiter = obj.get_waiter("cluster_available")
    ...
    def extract_cluster_properties(self):
        clusterProperties = obj.describe_clusters(ClusterIdentifier=self.name)[
            "Clusters"
        ][0]
        keysToShow = [
            "ClusterIdentifier",
    ...
    # TODO: Added new function to preint all values
    def extract_cluster_properties_new(self):
        clusterProperties = obj.describe_clusters(ClusterIdentifier=self.name)[
            "Clusters"
        ][0]
        ...
        x = [(k, v) for k, v in clusterProperties.items()]
        return clusterProperties.items()
    ...
    def execute_sql(self, sql, statementname):
        try:
            ...
            print(f"self: {self}")
            print("-" * 80)
            df = RedshiftEphemeral.extract_cluster_properties(self)
            print("-" * 80)
            print(f"df from execute_sql {df}")
            print("-" * 80)

            # TODO: Atempt to read the DBName from self automatically
            # Not working for now, so commenting
            # df = RedshiftEphemeral.extract_cluster_properties_new(self)
            # print(f"new df from execute_sql {df}")

            # print(f"DB Name from df: {df['DBName']}")
            # print(f"DB Name from self: {self.DBName}")
            # time.sleep(30)
            response = obj_data.execute_statement(
                ClusterIdentifier=self.name,
                # Database='dev', # TODO: why manual addition of dbname?
                Database="test_poc_1",
            ...
            print(
                f"result for obj_data.describe_statement(Id=response['Id']): {result}"
            )
            ...
            while check_status == True:
                ...
                if self.poll_status(response["Id"]) == "FINISHED":
                    ...
                    print(f"Response from pollstatus true: {result_output}")
                    return result_output
                if self.poll_status(response["Id"]) == "FAILED":
                    check_status = False
                    print(f"Response from pollstatus false")
                    return "execution failed"
                    # return results
        except Exception as e:
            print("Error in execution", e)
        end_time = time.time()
        print(f"Reached before time of execution print")
    ...
    def convert_results_to_df(self, result):
        try:
            if result != -1:
                if (
                    "TotalNumRows" in result
                    and "ColumnMetadata" in result
                ...
                for i in range(ncols):
                    col_labels.append(result["ColumnMetadata"][i]["label"])
                records = []
                for i in range(nrows):
                    records.append(resultrows[i])
                df = pd.DataFrame(
                    np.array(resultrows), columns=col_labels
